...
- P12.9: Smoke test + export fixes — (1) Proposal PDF and Build Report PDF now generated client-side via @react-pdf/renderer pdf().toBlob() (server-side renderToBuffer was crashing silently in dev); (2) JSON BoM generated client-side from live Zustand state (no DB roundtrip = always fresh); (3) DOCX and Build Report MD force-save via saveProjectAction() before hitting API route to eliminate autosave race condition; (4) PDF Preview tab removed from Export page — download buttons only; (5) BomTable price input now uses key={unitPriceUsd} to force remount after override, fixing stale defaultValue display; (6) Reset button moved below price input with "Reset" label for visibility; phase_plan_5.md P12.9 smoke test steps corrected (Dell XE9680 only supports H100/A100 — incompatibility test uses sequence: set H100+Dell, then change GPU to MI300X). Date: 2026-04-25
- P13.1–P13.19: Phase 13 — dynamic DB-backed catalogs. Drizzle schema migration (servers, gpus, llm_models, workload_references tables with origin tracking and append-only seed merge); seed file loader reading from JSON files at boot; catalog read API + in-process caching with 60s TTL; CatalogSnapshot interface decoupling sizing engine from DB (enables isomorphic test fixtures); catalog write API (create/update/deprecate/delete/resetToSeed for all four entities, origin transition seed→seed-edited); Settings → Catalogs index page with live counts; admin pages for Servers, GPUs, LLM Models, Workload References (table + add/edit dialog, deprecate, delete, reset-to-seed); catalog-extract LLM feature for URL-assisted entry (fetch spec page, LLM extracts fields, pre-fills dialog — Server, GPU, LLM Model only; Workload References manual-only); WorkloadReferences component at top of WorkloadForm (link-buttons, new-tab, hidden when empty); ExplainBox maximize button opens shadcn Dialog with same Explain+Example+Ask AI content (same call path, no separate routing); SidebarVersion component in sidebar footer showing v{version} with tooltip (build SHA + date from NEXT_PUBLIC env vars, "dev" label when absent). 127/127 tests passing. TypeScript clean throughout. Date: 2026-04-27
