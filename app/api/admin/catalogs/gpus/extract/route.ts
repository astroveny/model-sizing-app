...
import {
  CATALOG_EXTRACT_GPU_SYSTEM,
  buildCatalogExtractGpuPrompt,
} from "@/lib/llm/prompts/catalog-extract-gpu";
...
export async function POST(req: Request) {
  ...
  try {
    result = await provider.complete({
      system: CATALOG_EXTRACT_GPU_SYSTEM,
      messages: [{ role: "user", content: buildCatalogExtractGpuPrompt(page.url, page.text) }],
      maxTokens: 1000,
      json: true,
    });
  } catch (err) {
